...
class Algoritmo_comportamento_sensor():
    def __init__(self, dataset, qtd_train_inicial, tamanho_janela, passo, lags, qtd_neuronios, numero_particulas, n_particulas_comportamento, qtd_sensores, limite):
        '''
    ...
    def Atualizar_comportamento(self, vetor_caracteristicas, lags, enxame):
        ...
        '''
        Metodo para computar a deteccao de mudanca na serie temporal por meio dos sensores 
    ...
    def Executar(self, grafico = None):
        ...
        for x, i in enumerate(sensores):
            [i[0], i[1]] = self.Atualizar_sensores(janela_caracteristicas.dados, self.lags, enxame_vigente, enxame_vigente.sensores[x])
            ...
            if(i%self.passo == 0):
                ...
                if(contador == self.limite): 
                    ...
                    for x, j in enumerate(sensores_atuais):
                        ...
                        [j[0], j[1]] = self.Atualizar_sensores(janela_caracteristicas.dados[0], self.lags, enxame_vigente, enxame_vigente.sensores[x])
                        #print("[%d] %s" % (i, j[0]))
                    ...
                    if(all(ativadores)):    
                        ...
                        for x, i in enumerate(sensores):
                            [i[0], i[1]] = self.Atualizar_sensores(janela_caracteristicas.dados[0], self.lags, enxame_vigente, enxame_vigente.sensores[x])
                        sensores_atuais = copy.deepcopy(sensores)
                        ativadores = [False] * self.qtd_sensores
...
def main():
    ...
    dtst = Datasets()
    dataset = dtst.Leitura_dados(dtst.bases_nlinear_graduais(1, 30))
    #pt = Particionar_series(dataset, [1, 0, 0])
    #dataset = pt.Normalizar(dataset)
    
      
    #instanciando o algoritmo com sensores
    #dataset, qtd_train_inicial, tamanho_janela, passo, lags, qtd_neuronios, numero_particulas, n_particulas_comportamento, qtd_sensores, limite
    qtd_train_inicial = 1000
    tamanho_janela = 500 
    passo = 3
    lags = 4
    ...
    qtd_sensores = 1 
    limite = 1
    alg = Algoritmo_comportamento_sensor(dataset, qtd_train_inicial, tamanho_janela, passo, lags, qtd_neuronios, numero_particulas, n_particulas_comportamento, qtd_sensores, limite)
